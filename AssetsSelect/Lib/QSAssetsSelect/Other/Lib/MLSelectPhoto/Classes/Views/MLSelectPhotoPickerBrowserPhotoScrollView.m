...
- (void)setPhoto:(MLSelectPhotoAssets *)photo{
    _photo = (MLSelectPhotoAssets *)photo;
    
    // 这里传入的Image可能并不是一个MLSelectPhotoAssets， 有可能只是一个单纯的UIImage, 所以要加类型的判断 Sam Wang
    ...
    }else if ([photo isKindOfClass:[ALAsset class]]){
        @autoreleasepool {
            ALAsset *asset = (ALAsset *)photo;
            
            _photoImageView.image = [UIImage imageWithCGImage:[asset thumbnail]];
            
            [self displayImage];
        }
    }else if ([photo isKindOfClass:[MLSelectPhotoAssets class]]){
