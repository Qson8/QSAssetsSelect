...
- (void) jump2StatusVc{
    ...
    for (MLSelectPhotoPickerGroup *group in self.groups) {
        if ((self.status == PickerViewShowStatusCameraRoll || self.status == PickerViewShowStatusVideo) && ([group.groupName isEqualToString:@"Camera Roll"] || [group.groupName isEqualToString:@"相机胶卷"])) {
            ...
            gp = group;
            break;
        }else if ((self.status == PickerViewShowStatusCameraRoll) && ([group.groupName isEqualToString:@"All Photos"]||[group.groupName isEqualToString:@"All Photo"] || [group.groupName isEqualToString:@"所有照片"])) {
            gp = group;
            break;
        }
    }
