...
/// 裁剪视频导出路径
+ (void)exportVideoWithVideoAsset:(AVAsset *)videoAsset startTime:(CGFloat)startTime endTime:(CGFloat)endTime deviceDirection:(UIImageOrientation)deviceDirection completion:(void (^)(NSURL *outputPath, NSError *error, IJSVideoState state))completion
{
    // 设置裁剪时间
    ...
    //获取优化后的视频转向信息
    AVMutableVideoComposition *videoComposition = [self fixedCompositionWithAsset:videoAsset deviceDirection:deviceDirection];
...
/// 获取优化后的视频转向信息
+ (AVMutableVideoComposition *)fixedCompositionWithAsset:(AVAsset *)videoAsset deviceDirection:(UIImageOrientation)deviceDirection
{
    //1,可以用来对视频进行操作,用来生成video的组合指令，包含多段instruction
    AVMutableVideoComposition *videoComposition = [AVMutableVideoComposition videoComposition];
    // 视频转向
//    int degrees = [self degressFromVideoFileWithAsset:videoAsset];
    ...
    // 根据设备方向来处理视频旋转
    if (deviceDirection == UIImageOrientationUp)
    {
        // 顺时针旋转90°
        translateToCenter = CGAffineTransformMakeTranslation(videoTrack.naturalSize.height, 0.0);
        mixedTransform = CGAffineTransformRotate(translateToCenter, M_PI_2);
     ...
     if (deviceDirection == UIImageOrientationRight)
    {
        // 顺时针旋转180°
        translateToCenter = CGAffineTransformMakeTranslation(videoTrack.naturalSize.width, videoTrack.naturalSize.height);
        mixedTransform = CGAffineTransformRotate(translateToCenter, M_PI);
    ...
    else if (deviceDirection == UIImageOrientationDown)
    {
        // 顺时针旋转270°
        translateToCenter = CGAffineTransformMakeTranslation(0.0, videoTrack.naturalSize.width);
        mixedTransform = CGAffineTransformRotate(translateToCenter, M_PI_2 * 3.0);
    ...
    else if (deviceDirection == UIImageOrientationLeft)
    {
        // 顺时针旋转360°
        translateToCenter = CGAffineTransformMakeTranslation(0.0, 0.0);
        mixedTransform = CGAffineTransformRotate(translateToCenter, M_PI_2 * 4);
        videoComposition.renderSize = CGSizeMake(videoTrack.naturalSize.width, videoTrack.naturalSize.height);
        [roateLayerInstruction setTransform:mixedTransform atTime:kCMTimeZero];
    }
    ...
    if ([compatiblePresets containsObject:AVAssetExportPresetHighestQuality])
    {
        AVAssetExportSession *exportSession = [[AVAssetExportSession alloc]
            initWithAsset:videoAsset
               presetName:AVAssetExportPresetMediumQuality]; //AVAssetExportPresetPassthrough可能返回没有处理过的视频
        if (!isCut)
        {
