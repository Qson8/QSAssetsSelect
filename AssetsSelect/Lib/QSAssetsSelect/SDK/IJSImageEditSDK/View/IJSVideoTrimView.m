...
//    self.slideHeight = self.js_height * 0.7; // 滑块的高度
    self.slideHeight = self.js_height;
//    self.slideWidth = JSScreenWidth * 0.03;  // 滑块的宽度
    self.slideWidth = kIPadSuitFloat(18);
...
{
    // 背景滚动图
    UIScrollView *backScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, self.js_width, self.js_height - 0)];
    ...
    // 左滑块
    UIImage *image = [UIImage imageNamed:@"media_dotto"];
    IJSVideoSlideView *leftSlideView = [[IJSVideoSlideView alloc] initWithFrame:CGRectMake(self.leftOverlayView.js_width - self.slideWidth, 0, self.slideWidth, self.slideHeight) backImage:image isLeft:YES];
    ...
    // 右滑块
    image = [UIImage imageNamed:@"media_dotto"];
    IJSVideoSlideView *rightSlideView = [[IJSVideoSlideView alloc] initWithFrame:CGRectMake(0, 0, self.slideWidth, self.slideHeight) backImage:image isLeft:NO];
    [rightOverlayView addSubview:rightSlideView];
    self.rightSlideView = rightSlideView;

    //中间的条白线
    UIView *trackerView = [[UIView alloc] initWithFrame:CGRectMake(CGRectGetMaxX(leftOverlayView.frame), 0+1, 3, self.slideHeight - 2)];
    trackerView.backgroundColor = [UIColor lightGrayColor];
    [self addSubview:trackerView];
    self.trackerView = trackerView;

    //顶部线
    UIView *topBorder = [[UIView alloc] initWithFrame:CGRectMake(self.marginLeft, 0, CGRectGetMinX(self.rightOverlayView.frame) - CGRectGetMaxX(self.leftOverlayView.frame), 4)];
    topBorder.backgroundColor = [UIColor whiteColor];
    [self addSubview:topBorder];
    self.topBorder = topBorder;
    //底部的线
    UIView *bottomBorder = [[UIView alloc] initWithFrame:CGRectMake(topBorder.js_left, 0 + self.slideHeight - 4, topBorder.js_width, 4)];
...
{
    self.topBorder.frame = CGRectMake(CGRectGetMaxX(self.leftOverlayView.frame), self.topBorder.y, CGRectGetMinX(self.rightOverlayView.frame) - CGRectGetMaxX(self.leftOverlayView.frame), 4);
    self.bottomBorder.frame = CGRectMake(self.topBorder.js_left, self.bottomBorder.y, self.topBorder.js_width, 4);
