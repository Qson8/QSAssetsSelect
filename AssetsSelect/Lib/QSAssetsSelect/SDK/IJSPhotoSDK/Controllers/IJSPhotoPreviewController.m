...
@property (nonatomic, weak) UILabel *descrLabel;                  /* 提示 */
...
//    editButton.backgroundColor = [IJSFColor colorWithR:40 G:170 B:40 alpha:1];
    ...
    [finishButton setTitleColor:[IJSFColor colorWithR:135 G:135 B:135 alpha:0.5] forState:UIControlStateDisabled];
//    finishButton.backgroundColor = [IJSFColor colorWithR:40 G:170 B:40 alpha:1];
    ...
    CGFloat maxCut = ((IJSImagePickerController *) self.navigationController).maxVideoCut ?: kMAXTIME;
    describeLabel.text = [NSString stringWithFormat:@"只支持%g秒内的视频，需进行编辑",maxCut];
    describeLabel.font = [UIFont systemFontOfSize:12];
    describeLabel.textColor = [IJSFColor colorWithR:232 G:236 B:239 alpha:1];
    describeLabel.textAlignment = NSTextAlignmentCenter;
    [toolBarView addSubview:describeLabel];
    describeLabel.frame = CGRectMake(CGRectGetMaxX(editButton.frame) + 10, editButton.y,finishButton.x - (CGRectGetMaxX(editButton.frame) + 10) - 10 , editButton.height);
    _descrLabel = describeLabel;
    ...
    {
        self.rightButton.hidden = YES;
//        self.showCollectioView.scrollEnabled = NO; // 禁止翻页滚动
        NSTimeInterval duration = model.asset.duration;
        CGFloat maxCut = ((IJSImagePickerController *) self.navigationController).maxVideoCut ?: kMAXTIME;
        if(roundf(duration) > maxCut) {
            CGLog(@"大于%g秒",maxCut);
...
- (void)_finishSelectImageDisMiss
{
    ...
    if (vc.selectedModels.count == 0) // 用户没有选中图片或者视频
    {
        ...
        {
            [self _getBackThumbnailDataPhotos:photos assets:assets infoArr:infoArr avPlayers:avPlayers model:model index:0 networkAccessAllowed:YES noAlert:noShowAlert vc:vc];
        }
        else
        {
            // 不满足最小要求就警告
...
//       [self.lodingView removeFromSuperview];
        ...
        if (sourceType == IJSPVideoType)
        {
            ...
            [IJSVideoManager addWatermarkForVideoAsset:asset waterImage:nil describe:@"今日澳洲" completion:^(NSURL *outputPath, NSError *error, IJSVideoState state) {
